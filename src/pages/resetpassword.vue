...
export default {
  ...
  methods: {
    async handleSubmit() {
      ...
      console.log("Submitting reset password:", formData);
      try {
        const res = await api.post("/resetpassword", formData);
...
<template>
  <div class="bg-[#E0D2B6] min-h-screen flex items-center justify-center">
    <div class="bg-[#FFF7E2] p-8 rounded-2xl shadow-lg flex w-320 h-200">
      <div class="flex-1">
        <Logo />
        <h2 class="text-4xl font-semibold mb-6 ml-50">Reset Password</h2>
        ...
        >
          <svg
            class="w-6 h-6 mr-2 text-gray-700 group-hover:text-[#EA580C] transition-colors duration-150 ease-in-out"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          ...
          <span
            class="text-gray-700 group-hover:text-[#EA580C] transition-colors duration-150 ease-in-out"
            >I already have a Account</span
          >
        </router-link>
      </div>
